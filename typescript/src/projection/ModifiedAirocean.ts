...
export class ModifiedAirocean extends ConformalEstimate {
  ...
  public toGeo(x: number, y: number): number[] {
    let easia: boolean;
    if (y < 0) easia = x > 0;
    else if (y > ModifiedAirocean.ARC / 2)
      easia = x > (-ModifiedAirocean.ROOT3 * ModifiedAirocean.ARC) / 2;
    ...
    if (easia !== this.isEurasianPart(x, y)) return Airocean.OUT_OF_BOUNDS;
  ...
  protected static ARCTIC_M =
    (ModifiedAirocean.ARCTIC_Y -
      (ModifiedAirocean.ROOT3 * ModifiedAirocean.ARC) / 4) /
    (ModifiedAirocean.BERING_X - -0.5 * ModifiedAirocean.ARC);
  protected static ARCTIC_B =
    ModifiedAirocean.ARCTIC_Y -
    ModifiedAirocean.ARCTIC_M * ModifiedAirocean.BERING_X;
  ...
  protected static ALEUTIAN_M =
    (ModifiedAirocean.BERING_Y - ModifiedAirocean.ALEUTIAN_Y) /
    (ModifiedAirocean.BERING_X - ModifiedAirocean.ALEUTIAN_XR);
  protected static ALEUTIAN_B =
    ModifiedAirocean.BERING_Y -
    ModifiedAirocean.ALEUTIAN_M * ModifiedAirocean.BERING_X;

  protected isEurasianPart(x: number, y: number): boolean {
    if (x > 0) return false;
    if (x < -0.5 * ModifiedAirocean.ARC) return true;

    if (y > (ModifiedAirocean.ROOT3 * ModifiedAirocean.ARC) / 4)
      // above arctic ocean
      return x < 0;

    if (y < ModifiedAirocean.ALEUTIAN_Y)
      // below bering sea
      return y < ModifiedAirocean.ALEUTIAN_Y + ModifiedAirocean.ALEUTIAN_XL - x;

    if (y > ModifiedAirocean.BERING_Y) {
  ...
  public bounds(): number[] {
    return [
      -1.5 * ModifiedAirocean.ARC * ModifiedAirocean.ROOT3,
      -1.5 * ModifiedAirocean.ARC,
      3 * ModifiedAirocean.ARC,
      ModifiedAirocean.ROOT3 * ModifiedAirocean.ARC,
    ];
  }
}
