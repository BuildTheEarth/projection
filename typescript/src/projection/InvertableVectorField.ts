...
export class InvertableVectorField {
  ...
  public getInterpolatedVector(x: number, y: number): number[] {
    ...
    if (
      y < -InvertableVectorField.ROOT3 * (x - u1 - v1 - 1) ||
    ...
    return [
      ...
      (valx3 - valx1) * this.sideLength,
      (this.sideLength * flip * (2 * valx2 - valx1 - valx3)) /
        InvertableVectorField.ROOT3,
      (valy3 - valy1) * this.sideLength,
      (this.sideLength * flip * (2 * valy2 - valy1 - valy3)) /
        InvertableVectorField.ROOT3,
    ];
  }

  public applyNewtonsMethod(
    expectedf: number,
    expectedg: number,
    xest: number,
    yest: number,
